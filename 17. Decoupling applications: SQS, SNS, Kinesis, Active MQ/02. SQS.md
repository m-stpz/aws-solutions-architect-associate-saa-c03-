...
- Fully managed message queue where a component [producer] sends a message to a queue
  - key words for SQS:
    - decoupling, sudden spike loads, timeouts, scalability
...
- We've seen that we can have
  - SQS that our ec2 instances, within an ASG, poll
  - if the queue number gets too large, we can have CloudWatch monitor it and trigger and alarm
  - this alarm, in its turn, is used to tell ASG to scale up
    - this could also be to scale down

This pattern opens new possibilities

### 1. Queue-based load leveling | Buffer and Throttle pattern

- converts unpredictable, spiky write workloads into a steady, controlled stream of operations that you db comfortably digest
  - relational dbs have hard physical limits
    - max connections: depending the amount of connections, the connection pool might be exhausted
    - cost inefficiency: sizing a db cluster to survive a 1-min burst means paying for massive, idle compute 99% of the day
- you can modulate the writes to your dbs [Queue-based load leveling or Buffer and Throttle pattern]
- if you're facing an extremely high writing load in your db, you can can have SQS as a layer between your instances and the db

```
requests ---> [ASG | Enqueue messages] ---sends message ---> [SQS] <--polls messages--- [ASG | Dequeue message] --- insert--- [DB]
```

#### how this architecture works

1. ingestion layer: frontend ASG handles HTTP requests

- instead of writing to the db, it performs a lightweight, low-latency `sqs:SendMessage` API call and immediately responds to the client

2. buffer: SQS absorbds thousands of messages per second very easily

- the queue depth temporarily spikes [`ApproximateNumberOfMessagesVisible`]

3. controlled scaling: worker ASG scales via CloudWatch alarms, but with a strict `MaxSize` cap
4. rate-limit draining: even if there are 500k messages in the queue, only the capped worker fleet is draining them

- th db receives a flattened, predictable write rate until the queue is empty
