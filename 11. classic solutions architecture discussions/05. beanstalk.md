...
Match the use case with the environment

Instant cutover between completely separate environments without code redeployment

- Blue/green

Zero impact on live instances, completely clean environment, easiest and safest rollback with zero downtime

- immutable

Test a small percentage of real user traffic against the new code before a full rollout

- canary

Needs the fastest deployment time, and temporary downtime is acceptable

- all at once

Do you need zero downtime, but the application must maintain 100% capacity throughout the rollout
...
- Search beanstalk
- Select environment tier: web/worker
